...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $fileCSV = $_FILES["datiStudentiCSV"]["tmp_name"];
    ...
    try {
        ...
        while (!feof($file)) {
            if ($riga = fgetcsv($file, 0, ";") and $riga[1] !== "Email studente" and $riga[1] !== "") {
                //Inserisci i dati dello studente nel database:
                $query = "INSERT INTO Studenti (Email, Classe, Anno_scolastico) VALUES (?, ?, '" . $annoScolastico . "');";
                $stmt = $db->prepare($query);
                $stmt->execute([$riga[1], $riga[0]]);
            }
        }
